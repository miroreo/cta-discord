...
export const command: Command = {
  ...
  execute: async (b: Discord.Bot, i: Discord.Interaction, options) => {
    if (i.data!.customId?.startsWith("arrivals:refresh")) {
      const lastInteractionTimestamp = i.message!.editedTimestamp ||
        i.message!.timestamp;
      const secondsSinceLastInteraction = Math.floor(
        (Date.now() - lastInteractionTimestamp) / 1000,
      );
    ...
    if (stations.length > 25) {
      return await b.helpers.sendInteractionResponse(i.id, i.token, {
        ...
        data: {
          content:
            "Too many stations found for that query. Please refine your search.",
        },
      });
    }
...
async function generateBoard(
  ...
  const lastFewArrivals = arrivals.sort((a, b) =>
    a.arrivalTime.getTime() - b.arrivalTime.getTime()
  ).slice(0, 8);
  const arrivalBoard = await generateArrivalsBoard(
    `Upcoming Arrivals at ${station}`,
    lastFewArrivals,
  );
  return await b.helpers.editOriginalInteractionResponse(i.token, {
