...
import {
  ...
  getArrivalsForStop,
} from "../cta/arrivals.ts";
import { generateArrivalsBoard } from "../arrivaldisplay/gen.ts";

export const arrivalsForStation = async (
  b: Discord.Bot,
  ...
  if (stations.length > 1) {
    return await b.helpers.sendInteractionResponse(i.id, i.token, {
      ...
      data: {
        content:
          `Multiple stations found for that query. Please refine your search.\n\n${
  ...
  const arrivalBoard = await generateArrivalsBoard(
    `Upcoming Arrivals at ${station.name}`,
    lastFewArrivals,
  );
  return await b.helpers.sendInteractionResponse(i.id, i.token, {
    type: Discord.InteractionResponseTypes.ChannelMessageWithSource,
