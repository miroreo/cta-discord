import _stops from "./stops.json" with { type: "json" };
import _stations from "./stations.json" with { type: "json" };
...
const stations: {
  idFromName: { [key: string]: number };
  nameFromId: { [key: number]: string };
} = _stations;
...
export const searchStations = (
  ...
  if (stations.idFromName[query]) {
    return [{ id: stations.idFromName[query], name: query }];
  }
