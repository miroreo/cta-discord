...
const url =
  `http://lapi.transitchicago.com/api/1.0/ttpositions.aspx?key=${API_KEY}&rt=${all_routes}&outputType=JSON`;
...
export async function updatePositions() {
  ...
  if (data.ctatt.errNm !== null) {
    throw new Error(
      "Error thrown by CTA TrainTracker API. Error Number: " + data.ctatt.errNm,
    );
  }
  ...
  console.log(
    `${trainsOnLine(TrainLine.RED) == 0 ? "❌" : "🚇"} ${
      trainsOnLine(TrainLine.RED)
    } Red line`,
  );
  console.log(
    `${trainsOnLine(TrainLine.BLUE) == 0 ? "❌" : "🚇"} ${
      trainsOnLine(TrainLine.BLUE)
    } Blue line`,
  );
  console.log(
    `${trainsOnLine(TrainLine.GREEN) == 0 ? "❌" : "🚇"} ${
      trainsOnLine(TrainLine.GREEN)
    } Green line`,
  );
  console.log(
    `${trainsOnLine(TrainLine.BROWN) == 0 ? "❌" : "🚇"} ${
      trainsOnLine(TrainLine.BROWN)
    } Brown line`,
  );
  console.log(
    `${trainsOnLine(TrainLine.PURPLE) == 0 ? "❌" : "🚇"} ${
      trainsOnLine(TrainLine.PURPLE)
    } Purple line`,
  );
  console.log(
    `${trainsOnLine(TrainLine.YELLOW) == 0 ? "❌" : "🚇"} ${
      trainsOnLine(TrainLine.YELLOW)
    } Yellow line`,
  );
  console.log(
    `${trainsOnLine(TrainLine.PINK) == 0 ? "❌" : "🚇"} ${
      trainsOnLine(TrainLine.PINK)
    } Pink line`,
  );
  console.log(
    `${trainsOnLine(TrainLine.ORANGE) == 0 ? "❌" : "🚇"} ${
      trainsOnLine(TrainLine.ORANGE)
    } Orange line`,
  );
...
export function outOfPlaceTrains(positions: TrainPosition[]) {
  ...
  positions.forEach((train) => {
    ...
    if (train.nextStation.lines.indexOf(train.route) == -1) {
      if (
        train.route == TrainLine.PURPLE &&
        train.nextStation.lines.indexOf(TrainLine.PURPLE_EXPRESS) != -1
      ) return;
  ...
  trainPositions.forEach((train) => {
    ...
    if (train.nextStation.lines.indexOf(train.route) == -1) {
      ...
      console.log(
        ...
        } line train #${train.trainNumber}. Next Station ${train.nextStation.stopName}`,
      );
    }
  });
...
function checkRunNumbers() {
  ...
  trainPositions.forEach((train) => {
    ...
    switch (train.route) {
      case TrainLine.RED:
        if (train.trainNumber < 800 || train.trainNumber > 999) {
          susNumber = true;
        }
        break;
      case TrainLine.BLUE:
        if (train.trainNumber < 100 || train.trainNumber > 299) {
          susNumber = true;
        }
        break;
      case TrainLine.GREEN:
        if (
          train.trainNumber > 99 && train.trainNumber < 600 ||
          train.trainNumber > 699
        ) susNumber = true;
        break;
      case TrainLine.BROWN:
        if (train.trainNumber < 400 || train.trainNumber > 499) {
          susNumber = true;
        }
        break;
      case TrainLine.PURPLE:
        if (train.trainNumber < 500 || train.trainNumber > 590) {
          susNumber = true;
        }
        break;
      ...
      case TrainLine.YELLOW:
        if (train.trainNumber < 590 || train.trainNumber > 599) {
          susNumber = true;
        }
        break;
      case TrainLine.PINK:
        if (train.trainNumber < 300 || train.trainNumber > 399) {
          susNumber = true;
        }
        break;
      case TrainLine.ORANGE:
        if (train.trainNumber < 700 || train.trainNumber > 799) {
          susNumber = true;
        }
        break;
      default:
  ...
  positions.forEach((train) => {
    ...
    if (susNumber) {
      if (
        badTrains.filter((v) => {
          if (
            v.nextStation == train.nextStation &&
            v.trainNumber == train.trainNumber
          ) return true;
          return false;
        }).length == 0
      ) badTrains.push(train);
  ...
  trainPositions.forEach((train) => {
    if (train.isDelayed) {
      ...
      console.log(
        `🚇 ${
          utils.trainLineString(train.route)
        } Line Train ${train.trainNumber} is delayed, its next station should be ${train.nextStation?.stopName}`,
      );
    }
  });
}
