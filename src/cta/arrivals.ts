...
import { searchStations, stops } from "./stations.ts";
...
const parseArrival = (raw: RawArrival): Arrival => {
  const destination = getStop(parseInt(raw.destSt)) ||
    getStopByName(raw.destNm);
  // console.log(raw);
  return {
...
export const getArrivalsForStation = async (
  stationId: number,
): Promise<Arrival[]> => {
  const url =
    `http://lapi.transitchicago.com/api/1.0/ttarrivals.aspx?key=${CTA_API_KEY}&mapid=${stationId}&outputType=JSON`;
  const response = await fetch(url);
  const data: CTAArrivalsResponse = await response.json();
  if (parseInt(data.ctatt.errCd) !== 0) {
    discordLog.error(
      `Unknown API Error: ${data.ctatt.errNm} for station ${stationId}`,
    );
  }
  return data.ctatt.eta.map(parseArrival);
};

export const getArrivalsForStop = async (
  stopId: number,
): Promise<Arrival[]> => {
  const url =
    `http://lapi.transitchicago.com/api/1.0/ttarrivals.aspx?key=${CTA_API_KEY}&stpid=${stopId}&outputType=JSON`;
  const response = await fetch(url);
  const data: CTAArrivalsResponse = await response.json();
  if (parseInt(data.ctatt.errCd) !== 0) {
    discordLog.error(
      `Unknown API Error: ${data.ctatt.errNm} for station ${stopId}`,
    );
  }
  return data.ctatt.eta.map(parseArrival);
};
