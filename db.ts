import { loadEnv, PostgresClient } from "./deps.ts";
...
export const subscribeToAlerts = async (guildID: bigint, channelID: bigint) => {
  await client.queryArray({
    text:
      `UPDATE guilds SET has_alerts = true, alert_channel = $1 WHERE guild_id = $2`,
...
export const unsubscribeFromAlerts = async (guildID: bigint) => {
  await client.queryArray({
    text:
      `UPDATE guilds SET has_alerts = false, alert_channel = NULL WHERE guild_id = $1`,
    args: [guildID],
  }).catch((error) => {
    discordLog.error("Error pushing updated guild to database", error);
  });
...
export const pushAlertToDatabase = async (alert: AlertRecord) => {
  await client.queryArray({
    text:
      `INSERT INTO alert_history (alert_id, published_to, headline, short_description, guid) VALUES ($1, $2, $3, $4, $5)`,
    args: [
      alert.alertId,
      alert.publishedTo,
      alert.headline,
